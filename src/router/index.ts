...
import { Layer, Handlers } from "./layer";
import { mixin, setPrototypeOf } from "../utils";
...
export interface Router extends RouterOptions {
  ...
  process_params(layer: Layer, called: any, req: IncomingMessage, res: ServerResponse, done: any): void;
  use(...fn: Handlers[]): this;
  use(arg0: string, ...fn: Handlers[]): this;
  route(path: string): Route;
  param(name: string, fn: Handlers): this;
}



export function proto(options?: RouterOptions) {
  let opts: RouterOptions = options || {};

  const router: Router = ((req, res, next) => router.handle(req, res, next)) as any;
...
proto.param = function param(name, fn) {
  // param logic
  if (typeof name === 'function') throw new Error("router.param(fn): Refactor to use path params");
  ...
  if (name[0] === ':') throw new Error("router.param(" + JSON.stringify(name) + ", fn): Use router.param(" + JSON.stringify(name.slice(1)) + ", fn) instead");
...
proto.handle = function handle(this: Router, req: IncomingMessage, res: ServerResponse, out: any) {
  let self = this;
  ...
  function next(err?: any) {
    ...
    self.process_params(layer, paramcalled, req, res, function (err) {
      if (err) {
        ...
        layer.handle_request(req as any, res as any, next)
      } else {
...
proto.use = function use(fn) {
  ...
  let callbacks: Handlers[] = Array.from(slice.call(arguments, offset)).flat(Infinity) as any;
...
function restore(fn, obj, ...args: any[]) {
