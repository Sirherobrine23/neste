...
export interface Request extends http.IncomingMessage {
   ...
   *     // => "json"
   */
  accepts(...args: ([string, ...string[]])[]): string|boolean|any[];

  /**
   * Check if the given `encoding`s are accepted.
   */
  acceptsEncodings(...encoding: string[]): string|any[];

  /**
   * Check if the given `charset`s are acceptable,
   * otherwise you should respond with 406 "Not Acceptable".
   */
  acceptsCharsets(...charset: string[]): string|any[];

  /**
   * Check if the given `lang`s are acceptable,
   * otherwise you should respond with 406 "Not Acceptable".
   */
  acceptsLanguages(...langs: string[]): string|any[];
  ...
  param(name: string, ...args: unknown[]): string;
